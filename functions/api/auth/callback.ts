...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const errorRedirect = (message: string) =>
    Response.redirect(`${url.origin}/?error=${encodeURIComponent(message)}`)

  try {
    // Validate required environment variables
    if (!env.GITHUB_CLIENT_ID || !env.GITHUB_CLIENT_SECRET) {
      console.error('Missing GitHub OAuth credentials')
      return errorRedirect('Server configuration error: OAuth not configured')
    }
    if (!env.TOKEN_ENCRYPTION_KEY || env.TOKEN_ENCRYPTION_KEY.length !== 64) {
      console.error('Missing or invalid TOKEN_ENCRYPTION_KEY')
      return errorRedirect('Server configuration error: encryption not configured')
    }
    ...
    if (!userRes.ok) {
      return errorRedirect('failed_to_get_user')
    }
    ...
    if (user) {
      ...
      if (!isFirstUser) {
        ...
        if (!regMode || regMode.value === 'closed') {
          return errorRedirect('Registration is closed. Contact an administrator for access.')
        }
      }
    ...
    // Log the error for debugging (visible in Cloudflare dashboard)
    console.error('OAuth callback error:', err instanceof Error ? err.message : err)

    // Return user-friendly error redirect instead of crashing
